...
/**
 * BreathableCard: A card with an autonomous oscillator cycle.
 * Simulates the "resting" state of a high-end machine.
 */
const BreathableCard: React.FC<{ item: ServiceItem; index: number }> = ({ item, index }) => {
  const time = useTime();
  
  // Decoupled oscillator for "Resting Pulse"
  // Each card is offset by its index to prevent global synchronization
    ...
    >
      ...
      <div className="flex justify-between items-start relative z-10">
        <div className="w-14 h-14 rounded-2xl bg-accent/[0.03] border border-accent/5 flex items-center justify-center group-hover:bg-accent group-hover:border-accent transition-all duration-500">
          <span className="material-icons-outlined text-2xl text-accent group-hover:text-background transition-colors duration-500">
            {item.icon}
          </span>
        </div>
...
const Services: React.FC = () => {
  ...
  // Pulling items directly from the restored i18n schema
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <motion.div
            ...
            className="group p-10 rounded-[40px] bg-accent text-background border border-accent hover:shadow-[0_40px_100px_rgba(var(--accent-rgb),0.2)] transition-all duration-700 cursor-pointer min-h-[360px] flex flex-col justify-center items-center text-center relative overflow-hidden"
          >
            {/* Background Texture Overlay */}
            <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,_var(--background)_1px,_transparent_1px)] bg-[size:20px_20px]" />
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="inline-flex items-center gap-3 border-b border-background/20 pb-2 group-hover:border-background transition-colors">
